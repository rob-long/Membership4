...
var readability = {
	...
	articleContent.innerHTML = passedBodyContent;
    ...
    **/
    ...
    getArticleTitle: function () {
        ...
        if(typeof curTitle != "string") { /* If they had an element with id "title" in their HTML */
                curTitle = readability.getInnerText(document.getElementsByTagName('title')[0]);             
        }
        
        articleTitle.innerHTML = "<div>" + siteLogo + "</div>";

	if (headerText)
		articleTitle.innerHTML += "<div>" + headerText + "</div>";        

        if (hideTitle == '') 
        	articleTitle.innerHTML += "<h2>" + curTitle + "</h2>";
	...
	articleFooter.style.fontSize = "8pt";
	
	if (hideUrl == '')
        	articleFooter.innerHTML = "URL: <a target=\"_blank\" href=\"" + document.location.href + "\" style=\"font-size: 8pt;\">" + document.location.href + "</a><br/>";
	if (hideCopyright == '' && hideDate == '') {
		articleFooter.innerHTML += copyright +" - " + date;
	}
	else {
		if (hideCopyright == '')
			articleFooter.innerHTML += copyright;
		if (hideDate == '')
			articleFooter.innerHTML += date;
	}
    ...
    **/
    grabArticle: function (page) {
        ...
        **/
	    ...
	    // ************************** THIS IS WHERE I FORCE THE NODES TO BE REVIEWED **************************
            if (node.className == "maincontents") {
