...
sub show {
	...
	if (! $opt{readonly} && $this->allow("edit")) {
	    # check for tool use
	    $this->{input} = $in;
	    return $this->show_cmd($in->{cmd}) if ($in->{cmd});
	}
	$out .= $this->SUPER::show();
    }
    else {
...
sub configure {
	...
	my $ml = &get_obj("ML");
	my $out .= $ml->h1("Settings for ".$ml->em($this->title));
	my @hide = $this->{admin} ?
...
sub allow {
	...
	if ($user && $user->id == $this->uid) {
	    # allow user to update, edit their own profile
	    return 1 if ($op eq "update" || $op eq "edit");
	}
...
sub sale_note {
	...
	$this->wf_istatus() < $this->if_istatus("draft")) { # profile is submitted
